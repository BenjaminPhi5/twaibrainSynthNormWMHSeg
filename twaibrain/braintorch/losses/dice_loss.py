...
def individual_dice_v3(p_hat, y_true, epsilon=1e-6):
    p_hat = get_normalized_probs(p_hat)
    s0 = p_hat.shape[0]
    p_hat = p_hat.view(s0,-1)
    y_true = y_true.view(s0,-1)
    numerator = torch.sum(2. * p_hat * y_true, dim=1) + epsilon
    denominator = torch.sum(y_true.square() + p_hat.square(), dim=1) + epsilon
    combined = 1. - (numerator/denominator)
    return combined
...
class SoftDiceV3:
    ...
    def __call__(self, p_hat, y_true):
        return individual_dice_v3(p_hat, y_true, self.epsilon).mean()
