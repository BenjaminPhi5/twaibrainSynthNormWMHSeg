...
class Hierarchical_SSN_with_ConvSpatialAttention_V2(nn.Module):
    def __init__(self, 
                 base_unet, 
                 ssn_config,
                 refine_channels,
        ...
        self.out_channels = ssn_config['out_channels']
        self.upsample_stage = upsample_stage
        self.num_samples = num_samples

    def forward(self, x, symmetric=True, num_samples=None, rsample=True, separate_samples=False):
        # get main outputs from unet
        outputs = self.base_unet(x)
        mask = x[:, -1].unsqueeze(1)
        ...
        # get the standard segmentation output for stage 2 of the decoder
        decoder_s2_out = outputs[self.upsample_stage]
        mask_l = F.interpolate(mask, size=decoder_s2_out.shape[2:], mode='nearest')
        coarse_ssn = self.coarse_head(decoder_s2_out, mask_l)
        decoder_s2_mean = coarse_ssn['logit_mean']
        outputs[self.upsample_stage] = decoder_s2_mean

        for i in range(len(outputs)):
            if i == 0 or i == self.upsample_stage:
                continue
            outputs[i] = outputs[i][:, :self.out_channels]
        ...
        if separate_samples:
            ...
            outputs[0] = torch.cat([mean_head_out.unsqueeze(0), sample_outputs])

        return outputs

    def mean_and_sample(self, x, num_samples=10, rsample=True, symmetric=True):
        return self(x, symmetric=symmetric, num_samples=num_samples, rsample=rsample, separate_samples=True)[0]
